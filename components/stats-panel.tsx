...
export function StatsPanel({ detailed = false }: { detailed?: boolean }) {
  ...
  useEffect(() => {
    async function load() {
      ...
      const [sessionsRes, attemptsRes, cardsRes, grammarRes, scheduleRes] = await Promise.all([
        supabase.from("practice_sessions").select("*").eq("family_id", family.familyId).order("started_at", { ascending: false }).limit(50),
        supabase.from("practice_attempts").select("*").eq("family_id", family.familyId).order("created_at", { ascending: false }).limit(1000),
        ...
        supabase.from("review_schedule").select("*").eq("family_id", family.familyId).order("due_at", { ascending: true }).limit(100)
      ]);
  ...
  const stats = useMemo(() => {
    ...
    const weakestGrammar = Object.entries(byGrammar)
      .map(([grammarId, value]) => ({ grammar: grammarPatterns.find((item) => item.id === grammarId), ...value, rate: value.wrong / value.total }))
        ...
        <>
          ...
          <div className="mt-5 grid gap-5 lg:grid-cols-3">
            <Panel>
              ...
              <div className="grid gap-2">
                {stats.weakestCards.length ? stats.weakestCards.map((item) => (
            ...
            <Panel>
              ...
              {stats.weakestGrammar.length ? stats.weakestGrammar.map((item) => (
            ...
            <Panel>
              ...
              {dueToday.length ? dueToday.slice(0, 6).map((item) => {
